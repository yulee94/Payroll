...
pub(crate) fn scram_sha_256_salt(password: &[u8], salt: [u8; SCRAM_DEFAULT_SALT_LEN]) -> String {
    ...
    //
    // Compatibility rationale: follow the behavior of libpq's
    // PQencryptPasswordConn(), and also backend compatibility. If the password is not valid UTF-8, or if it
