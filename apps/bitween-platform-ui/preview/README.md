...
`npm run preview` starts the local static preview on port 4174; `npm run demo` is kept as the explicit demo wrapper.
...
- 현장사원: `worker` / `worker`
- 관리자: `office.admin` / `office`
- 임원: `executive` / `executive`
- 최고관리자: `admin` / `Dldsnckd94!`
- Company code for all preview accounts: `0000`

## Scope

- Mirrors the current login, launcher, sidebar, payroll, HR, attendance, 정비/렌탈, workflow, archive, AI, admin, and settings screen structure.
