...
This slice covers the supplied-profile predicates currently implemented in
`core.workflow.permissions`: site report visibility, month-close authority, and
execution-task management. Rust owns only deterministic decisions once callers
provide the principal, profile, site/task identifiers, and tenant context. Python
remains responsible for `UserSession` conversion, `get_user_profile`, workflow
JSON persistence, site/task storage lookups, close-month side effects, execution
task mutation, notifications, calendar/To-Do links, and UI bridge behavior.
...
/tmp/payroll-policy-venv/bin/python -m unittest tests.test_workflow_business_trip_contracts tests.test_workflow_permissions_contracts -v
git diff --check
...
RED/GREEN Rust operational permission tests and Python contract tests;
readability keeps predicates flat and reuses existing role/profile helpers;
architecture keeps Python as profile/task/site resolver and side-effect bridge;
...
- [x] RED Python contract test names the Rust operational permission boundary.
